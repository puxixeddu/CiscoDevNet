...
def main():
    ...
    device_hostname = input ('Enter the device to which deploying the template : ')
    project_name = template_info['project']
    template_file_name = template_info['template']

    # get a Cisco DNA Center auth token
    dnac_auth = dnac_apis3.get_dnac_jwt_token(DNAC_AUTH)

    # check if existing project, if not create a new project
    project_id = dnac_apis3.create_project(project_name, dnac_auth)
    if project_id == 'none':
    ...
    cli_valid = dnac_apis3.check_ipv4_duplicate(template_file_name)

    # upload and commit the template
    dnac_apis3.upload_template(cli_config_name, project_name, cli_config_commands, dnac_auth)

    if cli_valid is False:
        print('\n\nDeploying the CLI Template  "' + cli_config_name + '" will not create duplicated IP Addresses')
        # deploy the template
        deployment_id = dnac_apis3.send_deploy_template(cli_config_name, project_name, device_hostname, dnac_auth)
        print(deployment_id)
        time.sleep(6)
        deployment_status = dnac_apis3.check_template_deployment_status(deployment_id, dnac_auth)
